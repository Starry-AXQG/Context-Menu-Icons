...
// @description    An affiliated module of CMI, used for folding the context menu (icon-preserve patch)
// @author         Starry-AXQG + assistant patch
// @version        v2.1-mod-icon-preserve
// @include        main
// @grant          none
// ==/UserScript==
...
(function () {
  ...
  const iconStateMap = new Map(); // store previous inline style / class changes to restore
  ...
  // --- Icon preservation helpers (new) ---
  // Preserve icon-related presentation when moving elements between menus.
  // Strategy: capture previously-existing inline style; if computed styles show a background-image
  // or other icon hints, set inline styles (background-image, background-position, background-size, background-repeat)
  // and ensure iconic classes are present. Record what we changed so restore can revert exactly.
  function preserveIconBeforeMove(el) {
    try {
      if (!el || !el.id) return;
      const id = el.id;
      const prevStyle = el.getAttribute && el.getAttribute('style');
      const prevClasses = el.className || '';
      const added = { addedStyle: false, addedClassMenuItemIconic: false, prevStyle: prevStyle || null, prevClassName: prevClasses };

      // try to detect an icon via attributes commonly used in XUL
      const imageAttr = el.getAttribute && (el.getAttribute('image') || el.getAttribute('src') || el.getAttribute('icon'));
      ...
      let bgRepeat = null;
      if (computed) {
        try { bg = computed.getPropertyValue('background-image'); } catch (e) { bg = null; }
        try { bgPos = computed.getPropertyValue('background-position'); } catch (e) { bgPos = null; }
        try { bgSize = computed.getPropertyValue('background-size'); } catch (e) { bgSize = null; }
        try { bgRepeat = computed.getPropertyValue('background-repeat'); } catch (e) { bgRepeat = null; }
      }

      if (imageAttr) {
        if (el.classList && !el.classList.contains('menuitem-iconic')) { try { el.classList.add('menuitem-iconic'); added.addedClassMenuItemIconic = true; } catch (e) {} }
      }

      if (bg && bg !== 'none') {
        try {
          let add = '';
          add += 'background-image: ' + bg + ' !important; ';
          if (bgPos) add += 'background-position: ' + bgPos + ' !important; ';
          if (bgSize) add += 'background-size: ' + bgSize + ' !important; ';
          if (bgRepeat) add += 'background-repeat: ' + bgRepeat + ' !important; ';
          let newStyle = (prevStyle ? prevStyle + '; ' : '') + add;
          el.setAttribute('style', newStyle);
          added.addedStyle = true;
        } catch (e) { console.warn('[more-menu] preserveIcon: set inline style failed', e); }
        if (el.classList && !el.classList.contains('menuitem-iconic')) {
          try { el.classList.add('menuitem-iconic'); added.addedClassMenuItemIconic = true; } catch (e) {}
        }
      }

      iconStateMap.set(id, added);
    } catch (e) { console.warn('[more-menu] preserveIconBeforeMove error', e); }
  }

  function restoreIconAfterRestore(el) {
    try {
      if (!el || !el.id) return;
      const id = el.id;
      const record = iconStateMap.get(id);
      if (!record) return;
      try {
        if (record.prevStyle === null) {
          el.removeAttribute && el.removeAttribute('style');
        } else if (typeof record.prevStyle === 'string') {
          el.setAttribute && el.setAttribute('style', record.prevStyle);
        }
      } catch (e) { console.warn('[more-menu] restoreIcon: style restore failed', e); }
      try {
        if (record.prevClassName !== undefined && el.className !== undefined) {
          el.className = record.prevClassName;
        }
      } catch (e) { console.warn('[more-menu] restoreIcon: class restore failed', e); }
      iconStateMap.delete(id);
    } catch (e) { console.warn('[more-menu] restoreIconAfterRestore error', e); }
  }
  // --- end icon helpers ---
  ...
  function moveItemById(id, menupopup, allowClone = true) {
    ...
    // preserve icon/presentation before moving
    preserveIconBeforeMove(el);
    ...
    try {
      ...
      try {
        ...
        try {
          const rec = iconStateMap.get(id);
          if (rec && rec.prevStyle !== undefined) {
            const origStyle = el.getAttribute && el.getAttribute('style');
            if (origStyle) clone.setAttribute && clone.setAttribute('style', origStyle);
          }
          if (el.className) clone.className = el.className;
        } catch (ex) { console.warn('[more-menu] clone preserve class/style failed', ex); }

        morePopup.appendChild(clone);
        try { el.style.display = ''; el.classList && el.classList.add('sine-hidden-original'); } catch (ex) {}
        log('cloned and hid original', id);
        return true;
      } catch (ex) { console.error('[more-menu] clone failed for', id, ex); return false; }
    }
  }
  ...
  function restoreItemById(id) {
    ...
    try {
      ...
      try {
        ...
        // restore any inline styles/classes we added to preserve icons
        restoreIconAfterRestore(el);
        log('restored', id);
        return true;
      } catch (e) { console.error('[more-menu] restore failed for', id, e); return false; }
